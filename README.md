  ...
  2. Para criação de um Backend a ser desenvolvido em AspnetCore 8.0 C SHARP  
   ...
   2.2 - Projeto ASP.NET Core Web API  
     ...
     - _Pacotes instalados_:  
      ...
      - _Controllers_:
           * **AuthController** - Endpoint responsável pela entrada das credenciais do usuário (Username e Password), bloqueio, caso não esteja autenticado, ou geraçao do token JWT e envio/retorno para o client solicitante.  
           * **DevicesController** - Endpoint exposto a requisições que possuem credenciais validadas por meio do Token inseridos no Header da mesma.  
